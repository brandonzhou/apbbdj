...
import com.widget.adapters.AbstractWheelTextAdapter;
...
//                mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
    ...
    public void setTextViewStyle(String curriteItemText, CalendarTextAdapter adapter) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (curriteItemText.equals(currentText)) {
                ...
                textvew.setTextColor(mContext.getResources().getColor(R.color.grey_7));
            } else {
                textvew.setTextSize(MIN_TEXT_SIZE);
                textvew.setTextColor(mContext.getResources().getColor(R.color.grey_2));
            }
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sure_btn://确定选择按钮监听
                if (mBlnTimePickerGone) {
                 //   dateChooseInterface.getDateTime(strTimeToDateFormat(mYearStr, mDateStr), mBlnBeLongTerm);
                    dateChooseInterface.getDateTime(strHourAndMinuteFormat(mHourStr, mMinuteStr), mBlnBeLongTerm);
                } else {
            ...
            case R.id.long_term_tv://选择长期时间监听
                if (!mBlnBeLongTerm) {
                    mLongTermTextView.setBackgroundResource(R.drawable.green_bg);
                    mBlnBeLongTerm = true;
                } else {
                    mLongTermTextView.setBackgroundResource(R.drawable.green_bg);
    ...
    private String strHourAndMinuteFormat(String hour, String minute) {
