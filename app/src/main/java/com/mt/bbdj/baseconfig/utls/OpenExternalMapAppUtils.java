...
public class OpenExternalMapAppUtils {
    ...
    public static void openMapMarker(Activity activity, String longitude, String latitude,
                                     String title, String content, String appName,int type) {
        ...
        menuWindow = new SelectPopupWindow(activity, new SelectPopupWindow.OnPopWindowClickListener() {
            ...
            public void onPopWindowClickListener(View view) {
                if ( view.getId() == R.id.btn_select_baidu ) {
                    ...
                    //高德
                    LatLng latLng= TransBaiduGaodePoint.baidu_to_gaode(new LatLng(Double.parseDouble(longitude),Double.parseDouble(latitude)));
                    //高德
                    if ( type == TYPE_MAPVIEW_WITH_TIPS ) {
                        openGaodeMarkerMap(activity,latLng.latitude+"", latLng.longitude+"", appName, title);
                    } else if ( type == TYPE_MAPVIEW_DIRECTION ) {
                        openGaodeRouteMap(activity, latLng.latitude+"", latLng.longitude+"", title, dLongitude, dLatitude, dName, appName);
                    } else if ( type == TYPE_MAPVIEW_NAVI ) {
                        openGaodeNaviMap(activity, appName, title, latLng.latitude+"", latLng.longitude+"");
                    }
                }
            }
        }, showGaode, showBaidu);
