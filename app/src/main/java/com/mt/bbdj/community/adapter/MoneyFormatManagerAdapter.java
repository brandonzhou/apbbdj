 ...
 * @function
 */
public class MoneyFormatManagerAdapter extends
        RecyclerView.Adapter<MoneyFormatManagerAdapter.RegistrationObserverViewHolder> {
    ...
    private MoneyFormatManagerAdapter.OnRecyclerViewItemListener mOnRecyclerViewItemListener;

    public void setOnRecyclerViewItemListener(MoneyFormatManagerAdapter.OnRecyclerViewItemListener listener) {
        mOnRecyclerViewItemListener = listener;
    }

    public MoneyFormatManagerAdapter(Context context, List<HashMap<String,String>> data, LinearLayout head, int type) {
        this.data = data;
        this.mHead = head;
    ...
    public MoneyFormatManagerAdapter(Context context, List<HashMap<String,String>> data, int type) {
        this.data = data;
        this.mContext = context;
        this.type = type;
        notifyDataSetChanged();
    ...
    @Override
    public MoneyFormatManagerAdapter.RegistrationObserverViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(final MoneyFormatManagerAdapter.RegistrationObserverViewHolder holder, final int position) {
        ...
        holder.textView4.setText(map.get("send_name")+"/"+map.get("collect_name"));
        holder.textView5.setText(map.get("send_region")+"/"+map.get("collect_region"));
        holder.textView6.setText(map.get("goods_weight"));
        holder.textView7.setText(map.get("service_money"));
        holder.textView8.setText(map.get("deliver_money"));
    ...
    class OnScrollChangedListenerImp implements CustomHScrollView.OnScrollChangedListener {
        ...
        public void onScrollChanged(int l, int t, int oldl, int oldt) {
            ...
            if (n == 1) {//记录滚动的起始位置，避免因刷新数据引起错乱
                if (type == 1) {
                    ((MoneyFormatManagerActivity) mContext).setPosData(oldl, oldt);
                    notifyDataSetChanged();
                } else {
                    ((MoneyFormatManagerActivity) mContext).setPosData(oldl, oldt);
                }
