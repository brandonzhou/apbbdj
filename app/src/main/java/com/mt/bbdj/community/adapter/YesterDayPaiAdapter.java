...
import com.mt.bbdj.community.activity.YesterdPaiActivity;
 ...
 * Description :
 */
public class YesterDayPaiAdapter extends
        RecyclerView.Adapter<YesterDayPaiAdapter.RegistrationObserverViewHolder> {
    ...
    private YesterDayPaiAdapter.OnRecyclerViewItemListener mOnRecyclerViewItemListener;

    public void setOnRecyclerViewItemListener(YesterDayPaiAdapter.OnRecyclerViewItemListener listener) {
        mOnRecyclerViewItemListener = listener;
    }

    public YesterDayPaiAdapter(Context context, List<HashMap<String, String>> data, LinearLayout head, int type) {
        this.data = data;
        this.mHead = head;
    ...
    public YesterDayPaiAdapter(Context context, List<HashMap<String, String>> data, int type) {
        this.data = data;
        this.mContext = context;
        this.type = type;
        notifyDataSetChanged();
    ...
    @Override
    public YesterDayPaiAdapter.RegistrationObserverViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_title_2, parent, false);
        YesterDayPaiAdapter.RegistrationObserverViewHolder viewHolder = new YesterDayPaiAdapter.RegistrationObserverViewHolder(view);
        headSrcrollView = (CustomHScrollView) mHead.findViewById(R.id.h_scrollView);
        headSrcrollView.AddOnScrollChangedListener(new YesterDayPaiAdapter.OnScrollChangedListenerImp(viewHolder.scrollView));
        return viewHolder;
    }
    ...
    @Override
    public void onBindViewHolder(final YesterDayPaiAdapter.RegistrationObserverViewHolder holder, final int position) {
        ...
        holder.textView5.setText(map.get("warehousing_time"));
        holder.textView6.setText(map.get("out_time"));
        holder.textView7.setText(map.get("code"));
    ...
    class OnScrollChangedListenerImp implements CustomHScrollView.OnScrollChangedListener {
        ...
        public void onScrollChanged(int l, int t, int oldl, int oldt) {
            ...
            if (n == 1) {//记录滚动的起始位置，避免因刷新数据引起错乱
                if (type == 1) {
                    ((YesterdPaiActivity) mContext).setPosData(oldl, oldt);
                    notifyDataSetChanged();
                } else {
                    ((YesterdPaiActivity) mContext).setPosData(oldl, oldt);
                }
