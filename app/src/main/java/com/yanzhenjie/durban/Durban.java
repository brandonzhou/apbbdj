...
public class Durban {
    ...
    static final String KEY_ROTATE_SUPPORT = KEY_PREFIX + ".KEY_ROTATE_SUPPORT";

    static final String KEY_ROTATE_CONTEXT_NAME = KEY_PREFIX + ".KEY_ROTATE_CONTEXT_NAME";

    static final String KEY_ROTATE_OUT_ACTIVITY_NAME = KEY_PREFIX + ".KEY_ROTATE_OUT_ACTIVITY_NAME";

    static final String KEY_INTO_TYPE = KEY_PREFIX + ".KEY_INTO_TYPE";
     ...
     * 设置是否支持裁剪区域旋转
     *
     * @param isSupport   是否支持旋转
     * @param contextName  当前所在的环境
     * @return
     */
    public Durban rotateSupport(boolean isSupport,@NonNull String contextName) {
        mCropIntent.putExtra(KEY_ROTATE_SUPPORT, isSupport);
        mCropIntent.putExtra(KEY_ROTATE_CONTEXT_NAME, contextName);
        return this;
    }
    ...
    public void start() {
        try {
            ...
            method.invoke(o, mCropIntent, mCropIntent.getIntExtra("0", 1));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
