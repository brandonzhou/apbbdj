...
public final class ViewfinderView extends View {
    ...
    private static final int    SCANNER_LINE_MOVE_DISTANCE    =   12;  //扫描线移动距离
...
//        scannerAlpha = 0;
        ...
        mRect.right = screenWidth - getPaddingRight();
...
*/
        ...
        if(scannerStart == 0 || scannerEnd == 0) {
            scannerStart = mRect.top;
            scannerEnd = mRect.bottom - SCANNER_LINE_HEIGHT;
        }
        ...
        drawExterior(canvas,mRect,width,height);

        if (resultBitmap != null) {
            // Draw the opaque result bitmap over the scanning rectangle
            paint.setAlpha(CURRENT_POINT_OPACITY);
            canvas.drawBitmap(resultBitmap, null, mRect, paint);
        } else {
...
//            scannerAlpha = (scannerAlpha + 1) % SCANNER_ALPHA.length;
            ...
            drawFrame(canvas, mRect);
            // 绘制边角
            drawCorner(canvas, mRect);
            // Draw a red "laser scanner" line through the middle to show decoding is active
            drawLaserScanner(canvas,mRect);
            //绘制提示信息
            drawTextInfo(canvas, mRect);
            //绘制扫码结果点
            drawResultPoint(canvas,mRect);
